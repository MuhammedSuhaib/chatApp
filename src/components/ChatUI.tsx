      ...
      <main className="flex-1 overflow-y-auto space-y-4 p-2 rounded-lg bg-gradient-to-b from-white to-gray-100 dark:from-[#1a1441] dark:to-[#0d0c1d]">
        {messages.map((msg) => {
          ...
          return (
            <div
              key={msg.id}
              className={`flex items-end space-x-2 text-gray-900 dark:text-gray-100 ${
                isMine ? "justify-end" : "justify-start"
              }`}
            >
              <div
                className={`relative max-w-[80%] sm:max-w-[70%] ${
                  isMine ? "bg-[#20e07d]/20" : "bg-gray-200 dark:bg-neutral-700"
                      ...
                      <PopoverTrigger>
                        {isMine && msg.photoURL && (
                          <div className="flex justify-end-safe gap-3 items-center ">
                            <span className=" text-sm text-[#3f60f4]">
                              {msg.displayName}
                            </span>
                            <img
                              src={msg.photoURL}
